...
public class VoteUnbanCommandTests
{
    ...
    [Test]
    public async Task VoteUnbanAsync_RemovesBannedVoterFromDatabase()
    {
        ...
        await sut.VoteUnbanAsync(targetUser);

        dbContext.BannedVoters.Should().BeEmpty();
    }

    [Test]
    public async Task VoteUnbanAsync_UserNotBanned_DoesNotChangeDatabase()
    {
        var dbContext = TestHelpers.CreateVoteDbContext();
